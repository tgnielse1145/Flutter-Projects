...
// }
  ...
  Future<void> addContacts(Contact contact) async {
   ...
   // print("HERE IS CONTACTiD BEFORE ENCODE = "+ contact.id!);
    try {
  ...
  Future<void> getAndSetContacts([bool filterByUser = true]) async {
    ...
    try {
     ...
     // print("extracted_data "+extractedData.length.toString());
      final List<Contact> loadedContacts = [];
      extractedData.forEach((contId, contData) {
       // print("contId "+ contId!+ " contData "+ contData['name']!);
...
print('can it still get the contact id '+ id);
